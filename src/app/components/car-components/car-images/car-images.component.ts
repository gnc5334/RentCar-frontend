...
export class CarImagesComponent implements OnInit {
  ...
  getImagesByCarID(): void {
    this.imageService.getCarImageByCarId(this.car.id).subscribe((response) => {
  ...
  addImage(): void {
    if (this.carImageAddForm.valid) {
      ...
      formData.append('file', this.imageDataToUpload);
      formData.append('carId', this.car.id.toString());
      this.imageService.addImage(formData).subscribe(
        () => {
          this.toastrService.success('Görsel eklendi', 'Başarılı');
          this.reload();
        },
        (responseError) => {
  ...
  updateImage(event: any): void {
    ...
    if (this.carImageUpdateForm.valid && this.imageDataToUpload != null) {
      const formData: FormData = new FormData();
      formData.append('imagePath', this.imageDataToUpload);
  ...
  reload() {
    this.router
      ...
      .then(() => {
        this.router.navigate(['/carUpdate/' + this.car.id]);
      });
  }
